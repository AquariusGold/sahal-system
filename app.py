...
def admin_dashboard():
    ...
    activity_feed.extend({'timestamp': order.assigned_at, 'icon': 'user-check', 'tone': 'blue', 'title': 'Order assigned', 'description': f'Order #{order.id} assigned to {order.assigned_staff.full_name if order.assigned_staff else ''a removed staff account''}'} for order in Order.query.filter(Order.assigned_at.isnot(None)).order_by(Order.assigned_at.desc()).limit(5))
